...
int isFull(QueueType* cQ)
{
	...
	if (cQ->front == cQ->rear) { //만일 1칸 이동시킨 rear가 front와 같다면
		...
		return 0; //이동시킨rear != front 일때 0반환
}
...
void deQueue(QueueType* cQ, element* item)
{
	...
	//비어있다면 1 반환되어 바로 아래문장 실행 -> 함수종료
		return ;//함수종료
	
	cQ->front = ((cQ->front) + 1) % MAX_QUEUE_SIZE; //큐가 비어있지 않다면 front 1칸 증가(이동)
	//*item = cQ->queue[cQ->front];  = 증가한 front위치의 큐 값을 사용자의 명령이 들어가는 *item (data)에 초기화
	//  ㄴ 솔루션에서 참조한 위의 한줄의 코드는 원형 큐 구현에 있어서 어떤 용도인지 이해하지 못해 주석처리하였다.
	return; // 함수종료
}
void printQ(QueueType* cQ)
{
	...
	i = first;
	while (i != last) {//원형 큐에서는 front위치 칸은 비워놓기 때문에
	// 그 다음칸부터 rear 위치까지 출력후 그 다음칸에서 반복문 종료 
